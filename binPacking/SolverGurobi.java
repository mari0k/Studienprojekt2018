...
public class SolverGurobi {
	...
	public static void solveBinPacking(double[] zahlen, double binGröße) throws IOException, GRBException {

		int n = zahlen.length;

		for (int i = 0; i < n; i++) {
			if (zahlen[i] > binGröße) {
		...
		try {
			...
			for (int j = 0; j < n; j++) {
				...
				model.addConstr(expr, GRB.LESS_EQUAL, binGröße, "Bin Grenze eingehalten Bin" + j);
			}
